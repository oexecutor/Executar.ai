...
|---|---|---|
| Gratuito/demo | Workspace público atual, sem persistência garantida de longo prazo | Nunca — é a porta de entrada |
| Individual | Workspace próprio, persistência garantida, MCP completo | Só depois de sinal real de disposição a pagar no beta |
...
Nenhuma implementação de billing deve começar antes de uma decisão de
provedor (ex.: Stripe) e de pricing validado — isso é decisão do dono do
produto, não algo a inferir do código existente.
...
- Nenhuma ação de GTM aqui bloqueia ou é bloqueada pelas decisões técnicas
  pendentes (auth, persistência) registradas em
  `13-AUDITORIA-RIGOROSA-2026-07-28.md` §5 — elas correm em paralelo até o
  G6, onde auth real passa a ser pré-requisito.
