...
| RISK-004 | Teste e2e intermitente relacionado a `/icon.svg` | Média | Eliminada em 2026-07-28 | Ruído na CI e regressões mascaradas | Resolvido por mudança arquitetural (raiz virou landing pública distinta); 39/39 e 8/8 sem flakiness, ver `13-AUDITORIA-RIGOROSA-2026-07-28.md` §2 | Engenharia | — | Resolvido |
...
| RISK-011 | `vite build` (`emptyOutDir: true`) apagava silenciosamente `public/print/qr/*` e `public/blog/<slug>/cover.svg` — arquivos commitados fora de `web/public/`, incluindo páginas de QR já impressas fisicamente (PR #41) | Alta | Certa a cada deploy, até ser corrigido | Todo próximo build apagaria as páginas de QR impresso, gerando 404 para qualquer código físico escaneado | Reproduzido localmente e corrigido: arquivos movidos para `web/public/` (fonte real do Vite), sobrevivem a build limpo confirmado (`rm -rf public && npm run build`) | Engenharia | — | Resolvido |
